...
(async () => {
    ...
    await page.type('input[placeholder="e.g., Research Grant Review"]', 'MYTESTEVENT');
    ...
    if (text.includes("MYTESTEVENT")) {
        ...
        if (dayViewText.includes("MYTESTEVENT")) {
    ...
    console.log(await page.evaluate(() => {
        ...
        document.querySelectorAll('div').forEach(d => {
            if (d.innerText.includes("MYTESTEVENT")) { rs.push({ classes: d.className, html: d.outerHTML }); }
        });
        return rs;
    }));
